          ...
          <div className="flex items-center gap-4 flex-shrink-0">
            ...
            >
              <div className={`relative flex items-center ${isVibrating ? 'vibrating' : ''}`}>
                ...
                {getCartCount() > 0 && (
                  <span className="absolute -top-2 -right-3 bg-red-600 text-white text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center shadow-lg">{getCartCount()}</span>
                )}
              </div>
              <div className="flex flex-col items-start gap-0.5 hidden md:flex">
                <span className="text-sm font-bold text-black">{getCartTotal().toLocaleString('sv-SE')} kr</span>
